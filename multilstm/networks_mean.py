...
class StandardDeepLSTM(Network):
  ...
  def __init__(self, output_size, layers, preprocess_name="identity",
    ...
    self.counter = 0
    self.num_lstm = 5
    self.index = tf.placeholder(tf.int32, (self.num_lstm), name='index')
  ...
  def _build(self, inputs, prev_state):
    ...
    print(tf.gather(final_output_stack, self.index, axis=0))
  ...
  def initial_state_for_inputs(self, inputs, **kwargs):
    ...
    return self._rnns[0].initial_state(batch_size, **kwargs)
