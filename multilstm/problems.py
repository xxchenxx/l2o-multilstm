...
def mnist(layers,
  ...
  train_data = getattr(data, mode)
  images = tf.constant(train_data.images, dtype=tf.float32, name="MNIST_images")
  images = tf.reshape(images, [-1, 28, 28, 1])
  labels = tf.constant(train_data.labels, dtype=tf.int64, name="MNIST_labels")
  ...
  test_data = getattr(data, 'train')
  test_images = tf.constant(test_data.images, dtype=tf.float32, name="MNIST_test_images")
  test_images = tf.reshape(test_images, [-1, 28, 28, 1])
  test_labels = tf.constant(test_data.labels, dtype=tf.int64, name="MNIST_test_labels")
  def build():
